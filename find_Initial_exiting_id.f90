MODULE FindInitialExitingIdModule 
  IMPLICIT none
  CONTAINS

! Assuming y = B-1D, x = B-1b

SUBROUTINE find_Initial_exiting_id(Initial_d, Initial_n, y, x, enter_id, n, m, min_index)
  INTEGER, DIMENSION(128) :: Initial_d
  DOUBLE PRECISION, DIMENSION(128) :: x
  DOUBLE PRECISION, DIMENSION(128, 128) :: y
  INTEGER :: Initial_n, enter_id, n, m, min_index

  INTEGER :: i, j, init_flag, q
  DOUBLE PRECISION :: temp_min, temp

  DO i = 1, Initial_n
    IF (Initial_d(i) == enter_id) THEN
      q = i
    END IF
  END DO
...
END SUBROUTINE find_Initial_exiting_id

END MODULE FindInitialExitingIdModule